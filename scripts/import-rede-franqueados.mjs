...
if (!url || !serviceKey) {
  console.error("Defina NEXT_PUBLIC_SUPABASE_URL e SUPABASE_SERVICE_ROLE_KEY no .env.local (ou no ambiente).");
...
if (!csvPath) {
  console.error("Uso: node --env-file=.env.local scripts/import-rede-franqueados.mjs <arquivo.csv>");
  process.exit(1);
}
...
const COL_MAP = [
  ...
  ["regional", ["regional"]],
  ["area_atuacao", ["area de atuacao da franquia", "area de atuacao", "area atuacao", "área de atuação", "unidade"]],
  ...
  ["endereco_casa_frank", ["endereco casa do frank", "endereco casa frank", "endereço casa", "rua", "endereco", "complemento"]],
  ...
  ["tamanho_camisa_frank", ["tamanho da camiseta do frank", "tamanho camiseta", "tamanho camisa", "camiseta"]],
  ["socios", ["socios", "sócios", "listar nome nascimento"]],
  ["data_recebimento_kit_boas_vindas", ["data de recebimento do kit de boas vindas", "kit de boas vindas", "recebimento kit"]],
  ["ordem", ["ordem", "order"]],
];
