...
function CompletionBadge() {
  return (
    ...
    >
      ...
      <span className="text-sm font-semibold tracking-tight">Setup operacional concluído</span>
    </div>
  );
}
    ...
    <div className="w-full space-y-2">
      <div className="flex items-center justify-between gap-3">
        <p className="text-sm font-medium text-slate-700">
          Setup operacional — <span className="tabular-nums text-slate-900">{pct}</span>% concluído
        </p>
      </div>
      <div
        ...
        aria-valuemin={0}
        aria-valuemax={100}
        aria-label={`Setup operacional, ${pct} por cento concluído`}
      >
        <div
