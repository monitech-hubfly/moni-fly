...
export function PortalSidebar({ user, userRole }: PortalSidebarProps) {
  ...
  useEffect(() => {
    if (
      ...
      if (pathname.startsWith("/unidade-franquia") || pathname.startsWith("/due-diligence-empresas"))
        setUnidadeOpen(true);
    }
  }, [pathname]);
  ...
  return (
    ...
    >
      <div
        className={`flex h-14 items-center justify-between border-b px-4 gap-2 ${
      ...
      >
        <Link
          ...
          className={`text-lg font-semibold tracking-tight ${
            isSirene ? "text-stone-100 hover:text-white" : "text-moni-primary hover:text-moni-secondary"
          }`}
        >
          Moní
        </Link>
      ...
      <nav className="flex-1 space-y-1 p-3">
          ...
          <div>
            <div className="flex w-full items-center justify-between gap-1 rounded-lg border-l-2 border-transparent">
              <Link
                ...
                className={linkClassTituloBloco(pathname.startsWith("/processo-seletivo-candidatos"))}
              >
              ...
              <div className="ml-3 mt-1 space-y-0.5 border-l-2 border-moni-accent/30 pl-2">
                <Link
                  ...
                  className={linkClassNivel1(
                    pathname.startsWith("/processo-seletivo-candidatos/due-diligence-avaliacao-inicial"),
                  )}
                >
                ...
                <Link
                  ...
                  className={linkClassNivel1(pathname.startsWith("/processo-seletivo-candidatos/forms-cof"))}
                >
                  Forms + COF
                </Link>
                <Link
                ...
                <Link
                  ...
                  className={linkClassNivel1(pathname.startsWith("/processo-seletivo-candidatos/termo-scr"))}
                >
                  Termo SCR
                </Link>
              </div>
        ...
        <Link
          href="/rede"
          className={linkClassPrincipal(pathname.startsWith("/rede") && !pathname.startsWith("/rede-franqueados"))}
        >
          Rede de contatos
        </Link>
          ...
          {acoplamentoPlOpen && (
            <div className="ml-3 mt-1 space-y-0.5 border-l-2 border-moni-accent/30 pl-2">
              <Link
                ...
                className={linkClassNivel1(pathname.startsWith("/acoplamento-pl/modelagem-terreno"))}
              >
              ...
              <Link
                ...
                className={linkClassNivel1(pathname.startsWith("/acoplamento-pl/modelagem-casa-gbox"))}
              >
                Modelagem da Casa e GBox
              </Link>
              <Link
                href="/acoplamento-pl/validacao-acoplamento"
                className={linkClassNivel1(pathname.startsWith("/acoplamento-pl/validacao-acoplamento"))}
              >
                Validação do Acoplamento
              </Link>
              <Link
                href="/acoplamento-pl/alteracoes-acoplamento"
                className={linkClassNivel1(pathname.startsWith("/acoplamento-pl/alteracoes-acoplamento"))}
              >
        ...
        <Link
          ...
          className={
            isSirene && pathname.startsWith("/sirene")
              ? "block w-full rounded-lg px-3 py-2 text-left text-sm font-semibold transition bg-emerald-400 text-stone-900 hover:bg-emerald-300"
      ...
      <div className="border-t border-stone-200 p-3 space-y-1">
        <div className="flex w-full items-center gap-2 rounded-lg px-3 py-2">
          <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-moni-primary text-white text-xs font-semibold">
            {inicial}
          </span>
          <div className="min-w-0 flex-1">
            <span className="block text-sm font-semibold text-moni-primary truncate">
              {displayName}
            </span>
            {user?.email && (
              <span className="block text-[10px] text-stone-300 truncate mt-0.5">
                {user.email}
              </span>
            )}
          </div>
        ...
        <div>
          ...
          >
            ...
            <span className="text-moni-muted shrink-0">
              {perfilOpen ? (
                <ChevronDown className="h-3.5 w-3.5" />
              ) : (
                <ChevronRight className="h-3.5 w-3.5" />
                ...
                {frankOpen && (
                  <div className="ml-2 mt-0.5 space-y-0.5 border-l-2 border-moni-accent/20 pl-2">
                    ...
                    <Link
                      ...
                      className={`flex items-center gap-2 py-1.5 px-2 text-sm rounded ${
                        pathname.startsWith("/due-diligence-frank")
                          ? "bg-moni-light text-moni-secondary"
                          : "text-moni-muted hover:text-moni-secondary"
                      }`}
