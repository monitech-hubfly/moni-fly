  ...
  if (!processos.length) {
    return (
      <div className="mt-8 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">
        ...
        <p className="mt-2">Conclua as etapas 1 a 5 no Step 1 e use <strong>Finalizar estudo</strong> na etapa 5. Só estudos finalizados podem ser usados no Step 2.</p>
        <Link href="/step-one" className="mt-3 inline-block font-medium text-moni-accent hover:underline">
          Ir para Step 1 →
        </Link>
      </div>
    );
    ...
    <form onSubmit={handleContinuar} className="mt-8 space-y-4">
      <div>
        ...
        <p className="mt-1 text-xs text-stone-500">
          Selecione qual Step 1 (banco de informações) deseja utilizar. A listagem de casas e de lotes desse processo será usada no estudo; em seguida você fará a escolha do lote, dos 3 modelos, batalha, BCA e PDF.
        </p>
      </div>
      {error && (
