...
export default async function Step2ProcessoPage({ params }: PageProps) {
  ...
  const etapasStep2 = STEP2_ORDER.map(({ etapaId }) => ETAPAS.find((e) => e.id === etapaId)).filter(Boolean);
  ...
  return (
    <div className="min-h-screen bg-stone-50">
      ...
      <main className="mx-auto max-w-7xl px-4 py-8">
        <div className="rounded-xl border border-moni-primary/30 bg-moni-light/30 p-4 mb-8">
          <p className="text-xs font-semibold uppercase tracking-wide text-moni-primary">Step 1 em uso</p>
          <p className="mt-1 font-medium text-stone-900">
            {cidade || "Processo"}
            {estado ? `, ${estado}` : ""}
          </p>
          <p className="mt-1 text-sm text-stone-600">
            As listagens de lotes e de casas deste Step 1 já estão disponíveis. No Step 2 você só faz: escolha do lote, escolha dos 3 modelos e batalha, BCA e PDF. As etapas 1 a 5 do Step 1 não aparecem aqui.
          </p>
        </div>
        ...
        <p className="mt-1 text-stone-600">
          Primeira etapa: puxar o Step 1 (já feito acima). Agora: selecione o lote, depois use a listagem de casas existente com modelo e batalha, BCA e PDF.
        </p>

        <section className="mt-8">
          <h2 className="text-lg font-semibold text-stone-800 mb-3">Step 2</h2>
          <p className="text-sm text-stone-500 mb-3">A partir do Step 1 escolhido: escolha do lote, listagem (modelo + batalha), BCA e PDF</p>
          <ul className="space-y-3">
            {etapasStep2.map((etapa, idx) => (
              etapa && (
                <li key={etapa.id}>
                  <Link
        ...
        <div className="mt-8 rounded-xl border border-stone-200 bg-white p-4 text-sm text-stone-600">
          <strong>Resumo:</strong> Você puxou um Step 1 — os dados de análise, condomínios, resumo, listagem de lotes e listagem de casas já estão disponíveis. Aqui no Step 2 você só executa: escolha do lote (tabela de lotes), uso da listagem de casas já existente + escolha dos 3 modelos e batalha, BCA e PDF de hipóteses.
        </div>
      </main>
    </div>
  );
