...
export default function Step1Form() {
  ...
  const cidadesFiltradas = buscaCidade.trim()
    ? cidades.filter((c) =>
        c.nome.toLowerCase().includes(buscaCidade.trim().toLowerCase())
      )
    : cidades;
  ...
  return (
    <>
      <h1 className="moni-heading text-2xl">
        Iniciar Processo Step 1
      </h1>
      <p className="mt-2 text-stone-600">
        O Step 1 é o banco de informações por região/condomínio. Você aplica as etapas 1 a 5
        (análise da praça, condomínios, resumo, lotes e listagem de casas) e salva para usar no Step 2.
      </p>
          ...
          <div>
            ...
            <div className="mt-1 space-y-2">
              <input
                ...
                className="w-full rounded-lg border border-stone-300 px-3 py-2 text-sm disabled:bg-stone-100 disabled:text-stone-400 focus:border-moni-accent focus:outline-none focus:ring-1 focus:ring-moni-accent"
              />
              <select
                id="cidade"
                value={cidade}
              ...
              {errorCidades && (
                <p className="text-xs text-red-600">{errorCidades}</p>
              )}
            </div>
          </div>
        </div>
