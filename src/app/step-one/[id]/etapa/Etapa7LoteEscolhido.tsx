...
export function Etapa7LoteEscolhido({
  ...
  const persist = async () => {
    ...
    const result = await saveLoteEscolhido(processoId, {
      ...
      preco: preco
        ? parseFloat(String(preco).replace(/\./g, '').replace(',', '.')) || undefined
        : undefined,
      preco_m2: precoM2
        ? parseFloat(String(precoM2).replace(/\./g, '').replace(',', '.')) || undefined
        : undefined,
    });
    setLoading(false);
    if (result.ok) {
  ...
  return (
    <div className="mt-6 space-y-6">
      <p className="text-sm text-stone-600">
        ...
        . Selecione um lote da lista (Etapa 4) e preencha as informações adicionais abaixo.
      </p>

      {lotesDisponiveis.length > 0 && (
        <div>
          <h3 className="mb-2 font-medium text-stone-800">
            Lotes disponíveis (cadastrados na Etapa 4)
          </h3>
          <p className="mb-3 text-xs text-stone-500">
            Clique em &quot;Escolher este lote&quot; para preencher o formulário com os dados
            básicos. Depois complete recuos, localização, topografia e medidas.
          </p>
          <ul className="space-y-2">
            {lotesDisponiveis.map((l) => (
              <li
                key={l.id}
                className="flex flex-wrap items-center justify-between gap-2 rounded-lg border border-stone-200 bg-stone-50 p-3"
              >
                <span className="text-sm">
                  {l.condominio || '—'} — {l.area_lote_m2 ?? '—'} m² — R${' '}
                  {l.preco != null ? l.preco.toLocaleString('pt-BR') : '—'}
                  {l.link && (
                    <a
                      href={l.link}
                      target="_blank"
                      rel="noreferrer"
                      className="ml-2 text-moni-accent hover:underline"
                    >
                      Link
                    </a>
                  )}
      ...
      {lotesDisponiveis.length === 0 && (
        <p className="rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900">
          Nenhum lote cadastrado ainda. Volte à <strong>Etapa 4</strong> para adicionar lotes à
          venda; depois retorne aqui para escolher um e preencher as informações adicionais.
        </p>
      )}

      <h3 className="font-medium text-stone-800">Informações do lote escolhido</h3>
      <p className="mb-3 text-xs text-stone-500">
        As alterações são salvas automaticamente após você parar de editar.
      </p>
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          ...
          <div>
            ...
            <input
              type="text"
              value={recuos}
              onChange={(e) => setRecuos(e.target.value)}
              placeholder="Ex: 5m frente, 3m laterais"
              className="mt-1 w-full rounded-lg border border-stone-300 px-3 py-2 text-sm"
            />
          </div>
          <div className="sm:col-span-2">
            ...
            <input
              ...
              placeholder="Ex: perto da portaria, meio do condomínio"
              className="mt-1 w-full rounded-lg border border-stone-300 px-3 py-2 text-sm"
            />
          </div>
          <div>
