...
import { addLoteListing, deleteLoteListing, saveZapItemsEtapa5, saveLoteEscolhidoEtapa4 } from "./actions";
...
export function Etapa5Lotes(props: {
  ...
  const { processoId, lotes, loteEscolhidoId, cidadeInicial, estadoInicial, listagemOnly = false } = props;
  ...
  const handleSubmitManual = async (e: React.FormEvent) => {
    ...
    const valorCondNum = valorCondominio ? parseFloat(String(valorCondominio).replace(",", ".")) : undefined;
    ...
    <div className="mt-6 space-y-8">
      ...
      {lotes.length > 0 && (
        <section className="rounded-xl border border-stone-200 overflow-hidden">
          {!listagemOnly && (
            <div className="bg-stone-50 border-b border-stone-200 px-4 py-3 flex flex-wrap items-center gap-3">
              <p className="text-sm font-medium text-stone-800">
                Escolha <strong>1 lote</strong> da listagem para o estudo de viabilidade (marque na tabela e confirme).
              </p>
              {loteEscolhidoId && (
          ...
          <table className="w-full min-w-[500px] text-sm">
            <thead>
              <tr className="border-b border-stone-200 bg-stone-100">
                {!listagemOnly && <th className="p-2 text-left w-10">Escolher</th>}
            ...
            <tbody>
              {lotesPaginated.map((l) => (
                <tr key={l.id} className={`border-b border-stone-100 hover:bg-stone-50 ${!listagemOnly && selectedLoteId === l.id ? "bg-emerald-50/50" : ""}`}>
                  {!listagemOnly && (
                    <td className="p-2">
                  ...
                  <td className="p-2 text-right">{l.area_lote_m2 != null ? l.area_lote_m2.toLocaleString("pt-BR") : "—"}</td>
                  <td className="p-2 text-right">{l.preco != null ? `R$ ${l.preco.toLocaleString("pt-BR")}` : "—"}</td>
                  <td className="p-2 text-right">{l.preco_m2 != null ? `R$ ${l.preco_m2.toLocaleString("pt-BR")}` : "—"}</td>
                  <td className="p-2 text-right">{l.valor_condominio != null ? `R$ ${l.valor_condominio.toLocaleString("pt-BR")}/mês` : "—"}</td>
                  <td className="p-2 text-right">{l.iptu != null ? `R$ ${l.iptu.toLocaleString("pt-BR")}` : "—"}</td>
                  <td className="p-2 max-w-[200px] truncate" title={(l.caracteristicas ?? l.caracteristicas_condominio) ?? undefined}>{(l.caracteristicas ?? l.caracteristicas_condominio) ?? "—"}</td>
                  <td className="p-2">
                    {l.link ? (
                      <a href={l.link} target="_blank" rel="noreferrer" className="text-moni-accent hover:underline">Abrir anúncio</a>
                    ) : "—"}
                  </td>
                  <td className="p-2">
                    {l.manual ? (
      ...
      <div className="rounded-xl border border-stone-200 bg-stone-50 overflow-hidden">
        ...
        {manualFormOpen && (
          <form onSubmit={handleSubmitManual} className="p-4 pt-0 space-y-3 border-t border-stone-200">
            <p className="text-sm text-stone-600">Campos específicos para lotes/terrenos. Lotes adicionados manualmente só podem ser alterados ou excluídos por aqui (não vêm da ZAP).</p>
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <label className="grid gap-1">
                ...
                <input type="number" step="0.01" min={0} value={areaLote} onChange={(e) => setAreaLote(e.target.value)} className="rounded-lg border border-stone-300 px-3 py-2 text-sm" />
              </label>
              <label className="grid gap-1">
                <span className="text-sm font-medium text-stone-700">Preço (R$)</span>
                <input type="number" step="0.01" min={0} value={preco} onChange={(e) => setPreco(e.target.value)} className="rounded-lg border border-stone-300 px-3 py-2 text-sm" />
              </label>
              <label className="grid gap-1">
                <span className="text-sm font-medium text-stone-700">$ Condomínio (R$/mês)</span>
                <input type="number" step="0.01" min={0} value={valorCondominio} onChange={(e) => setValorCondominio(e.target.value)} className="rounded-lg border border-stone-300 px-3 py-2 text-sm" placeholder="Ex.: 1450" />
              </label>
              <label className="grid gap-1">
                <span className="text-sm font-medium text-stone-700">IPTU (R$)</span>
                <input type="number" step="0.01" min={0} value={iptu} onChange={(e) => setIptu(e.target.value)} className="rounded-lg border border-stone-300 px-3 py-2 text-sm" placeholder="Ex.: 900" />
              </label>
              <label className="grid gap-1 sm:col-span-2">
                <span className="text-sm font-medium text-stone-700">Características do condomínio</span>
                <input type="text" value={caracteristicasCondominio} onChange={(e) => setCaracteristicasCondominio(e.target.value)} className="rounded-lg border border-stone-300 px-3 py-2 text-sm" placeholder="Ex.: Salão de festas, Piscina, Academia, Quadra" />
              </label>
              <label className="grid gap-1 sm:col-span-2">
                <span className="text-sm font-medium text-stone-700">Link (opcional)</span>
                <input type="url" placeholder="URL do anúncio" value={link} onChange={(e) => setLink(e.target.value)} className="rounded-lg border border-stone-300 px-3 py-2 text-sm" />
              </label>
            </div>
            {error ? <p className="text-sm text-red-600">{error}</p> : null}
            <button type="submit" disabled={loading} className="btn-primary text-sm">{loading ? "Salvando…" : "Adicionar lote"}</button>
          </form>
        )}
      </div>
    </div>
