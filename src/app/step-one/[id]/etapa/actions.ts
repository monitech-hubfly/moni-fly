...
export async function saveEtapa1(
  ...
  const dadosJson =
    data.narrativa !== undefined
      ? { ...currentJson, narrativa: data.narrativa }
      : undefined;
...
export async function buscarCondominiosViaZap(processoId: string): Promise<ActionResult> {
  ...
  if (byCondo.size === 0) {
    return { ok: false, error: "Nenhum condomínio encontrado na ZAP para esta cidade (casas acima de 5 MM)." };
  }
...
export async function saveEtapa2(processoId: string, data: { concluida?: boolean }): Promise<SaveEtapa1Result> {
  const supabase = await createClient();
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) return { ok: false, error: "Faça login." };
...
export async function addCasaListing(processoId: string, data: {
  cidade?: string;
  estado?: string;
...
export async function saveZapItemsEtapa4(
  ...
  const { inserted, updated, despublicados } = await applyZapCasasUpdate(supabase, processoId, items, cidade, estado);
  return { ok: true, inserted, updated, despublicados };
}

export async function updateCasaCompatibilidadeMoni(casaId: string, compatibilidade_moni: string | null): Promise<ActionResult> {
  const supabase = await createClient();
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) return { ok: false, error: "Faça login." };
  const { error } = await supabase
...
export async function updateCasaStatus(casaId: string, status: "a_venda" | "despublicado"): Promise<ActionResult> {
  const supabase = await createClient();
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) return { ok: false, error: "Faça login." };
...
export type RunZapEtapa5Result =
...
export async function addLoteListing(processoId: string, data: {
  condominio?: string;
  area_lote_m2?: number;
  preco?: number;
  link?: string;
...
export async function saveCasasEscolhidasEtapa5(
  ...
  const { error: insEscolhidas } = await supabase
    .from("casas_escolhidas_etapa5")
    .insert(inserts);
...
export async function saveBatalhaCasasEtapa5(
  ...
  if (cleaned.length === 0) {
    return { ok: false, error: "Preencha pelo menos um critério (Atributos do Lote, Preço ou Produto) antes de salvar a batalha." };
  }
...
export async function saveScoreBatalhaPdfUrl(processoId: string, pdfUrl: string): Promise<ActionResult> {
  const supabase = await createClient();
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) return { ok: false, error: "Faça login." };
...
export async function deleteLoteListing(processoId: string, loteId: string): Promise<ActionResult> {
  ...
  if (row.manual !== true) return { ok: false, error: "Só é possível excluir lotes adicionados manualmente." };
  const { error } = await supabase
    .from("listings_lotes")
    .delete()
    .eq("id", loteId)
...
export async function saveLoteEscolhido(processoId: string, data: {
  cidade?: string;
  condominio?: string;
...
export async function saveCatalogoEscolhidos(
  ...
  if (uniq.length !== 3) return { ok: false, error: "Selecione exatamente 3 modelos do catálogo diferentes." };
...
export async function saveEtapa9(
  ...
  if (data.justificativas !== undefined) updates.dados_json = { ...currentJson, justificativas: data.justificativas };
  if (data.concluida) updates.concluida_em = new Date().toISOString();
  if (!row?.iniciada_em) updates.iniciada_em = new Date().toISOString();
...
export async function getBcaInputs(processoId: string): Promise<Partial<BcaInputs> | null> {
  ...
  const keys: (keyof BcaInputs)[] = [
    "nome_condominio", "nome_casa", "area_vendas_m2", "custo_terreno", "itbi_percentual",
    "custo_casa", "mes_inicio_obra",
    "obra_mes1", "obra_mes2", "obra_mes3", "obra_mes4", "obra_mes5", "obra_mes6", "obra_mes7", "obra_mes9", "obra_mes10",
    "comissao_vendas", "impostos", "taxa_plataforma", "taxa_gestao_frank", "projetos_taxa_obra", "capital_giro_inicial",
    "vgv_target", "vgv_liquidacao", "vgv_recompra",
    "permuta_planta", "permuta_target", "permuta_liquidacao", "permuta_recompra",
    "percentual_funding", "cdi_an",
  ];
  for (const k of keys) {
    if (row[k] !== undefined && row[k] !== null) (out as Record<string, unknown>)[k] = Number(row[k]) === row[k] ? Number(row[k]) : row[k];
  }
  return out;
}
...
export async function saveBcaInputs(
  ...
  const row: Record<string, unknown> = { processo_id: processoId, updated_at: new Date().toISOString() };
  const keys: (keyof BcaInputs)[] = [
    "nome_condominio", "nome_casa", "area_vendas_m2", "custo_terreno", "itbi_percentual",
    "custo_casa", "mes_inicio_obra",
    "obra_mes1", "obra_mes2", "obra_mes3", "obra_mes4", "obra_mes5", "obra_mes6", "obra_mes7", "obra_mes9", "obra_mes10",
...
export async function cancelarProcesso(processoId: string): Promise<ActionResult> {
  ...
  if (stepAtual < 2) return { ok: false, error: "Cancelamento permitido apenas a partir do Step 2." };
  if ((row as { cancelado_em?: string }).cancelado_em) return { ok: false, error: "Processo já está cancelado." };
...
export async function avancarParaProximoStep(processoId: string, proximoStep: number): Promise<ActionResult> {
  const supabase = await createClient();
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) return { ok: false, error: "Faça login." };
