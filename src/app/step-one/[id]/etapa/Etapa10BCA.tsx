...
function ValCell({ v, isPct = false, bold = false }: { v: number; isPct?: boolean; bold?: boolean }) {
  const text = isPct ? fmtPct(v) : fmtMoney(v);
  const isNeg = v < 0;
  return (
    <span
      className={
        bold ? "font-bold" : isNeg ? "text-red-600" : "text-green-700"
      }
    >
      {text}
    </span>
...
const OBRA_MES_KEYS = ["obra_mes1", "obra_mes2", "obra_mes3", "obra_mes4", "obra_mes5", "obra_mes6", "obra_mes7"] as const;
...
export function Etapa10BCA({
  ...
  const margemBadge =
    margem >= 10 ? "bg-green-100 text-green-800" : margem >= 5 ? "bg-amber-100 text-amber-800" : "bg-red-100 text-red-800";
  ...
  return (
    <div className="mt-6 space-y-8">
      ...
      <section className="rounded-xl border border-stone-200 bg-white p-4">
        <h3 className="text-lg font-semibold text-stone-800 mb-3">Identificação</h3>
      ...
      {/* BLOCO 2: Terreno */}
      <section className="rounded-xl border border-stone-200 bg-white p-4">
        <h3 className="text-lg font-semibold text-stone-800 mb-3">Terreno</h3>
        <div className="grid gap-3 sm:grid-cols-2">
          <div>
            <label className="block text-sm font-medium text-stone-700">Custo do terreno (R$)</label>
            <input
              type="number"
              className="mt-1 w-full rounded border border-stone-300 px-2 py-1.5 text-sm"
          ...
          <p className="text-sm text-stone-500 self-end">ITBI: 4% (fixo)</p>
        </div>
      </section>

      {/* BLOCO 3: Casa */}
      <section className="rounded-xl border border-stone-200 bg-white p-4">
        <h3 className="text-lg font-semibold text-stone-800 mb-3">Casa</h3>
        <div className="grid gap-3 sm:grid-cols-2 md:grid-cols-3">
          <div>
          ...
          <div>
            <label className="block text-sm font-medium text-stone-700">Mês de início da obra (1-48)</label>
            <input
              type="number"
              min={1}
              max={48}
        ...
        <div className="mt-4">
          <label className="block text-sm font-medium text-stone-700 mb-2">Fluxo de obra (%)</label>
      ...
      <section className="rounded-xl border border-stone-200 bg-white p-4">
        <h3 className="text-lg font-semibold text-stone-800 mb-3">Taxas e despesas</h3>
        <div className="grid gap-3 sm:grid-cols-2 md:grid-cols-3">
          <div>
            <label className="block text-sm font-medium text-stone-700">Comissão de vendas (%)</label>
            <input
              type="number"
              step={0.1}
              min={0}
              className="mt-1 w-full rounded border border-stone-300 px-2 py-1.5 text-sm"
              value={Math.abs((inputs.comissao_vendas ?? 0) * 100).toFixed(1).replace(".", ",")}
              onChange={(e) => updateInput("comissao_vendas", -(Number(e.target.value.replace(",", ".")) || 0) / 100)}
            />
          </div>
          <div>
            ...
            <input
              ...
              onChange={(e) => updateInput("impostos", (Number(e.target.value.replace(",", ".")) || 0) / 100)}
            />
          </div>
          <div>
            <label className="block text-sm font-medium text-stone-700">Taxa de plataforma (%)</label>
            <input
              type="number"
              step={0.1}
              min={0}
              className="mt-1 w-full rounded border border-stone-300 px-2 py-1.5 text-sm"
              value={Math.abs((inputs.taxa_plataforma ?? 0) * 100).toFixed(1).replace(".", ",")}
              onChange={(e) => updateInput("taxa_plataforma", -(Number(e.target.value.replace(",", ".")) || 0) / 100)}
            />
          </div>
          <div>
            <label className="block text-sm font-medium text-stone-700">Taxa de gestão Frank (%)</label>
            <input
              type="number"
              step={0.1}
              min={0}
              className="mt-1 w-full rounded border border-stone-300 px-2 py-1.5 text-sm"
              value={Math.abs((inputs.taxa_gestao_frank ?? 0) * 100).toFixed(1).replace(".", ",")}
              onChange={(e) => updateInput("taxa_gestao_frank", -(Number(e.target.value.replace(",", ".")) || 0) / 100)}
            />
          </div>
          <div>
            <label className="block text-sm font-medium text-stone-700">Projetos e taxa de obra (R$)</label>
            <input
              type="number"
              className="mt-1 w-full rounded border border-stone-300 px-2 py-1.5 text-sm"
              value={inputs.projetos_taxa_obra != null ? Math.abs(Number(inputs.projetos_taxa_obra)) : ""}
          ...
          <div>
            ...
            <input
              type="number"
              className="mt-1 w-full rounded border border-stone-300 px-2 py-1.5 text-sm"
              value={inputs.capital_giro_inicial != null ? Math.abs(Number(inputs.capital_giro_inicial)) : ""}
      ...
      {/* BLOCO 5: Cenários */}
      <section className="rounded-xl border border-stone-200 bg-white p-4">
        <h3 className="text-lg font-semibold text-stone-800 mb-3">Cenários</h3>
        <div className="overflow-x-auto">
          <table className="w-full min-w-[600px] text-sm">
            <thead>
              <tr className="border-b border-stone-200">
                <th className="text-left py-2 pr-4 font-medium text-stone-700"></th>
                <th className="text-right py-2 px-2 font-medium text-stone-700">Planta</th>
                <th className="text-right py-2 px-2 font-medium text-stone-700">Target</th>
                <th className="text-right py-2 px-2 font-medium text-stone-700">Liquidação</th>
                <th className="text-right py-2 px-2 font-medium text-stone-700">Recompra</th>
              </tr>
            </thead>
            <tbody>
              <tr className="border-b border-stone-100">
                <td className="py-1.5 pr-4 text-stone-600">VGV (R$)</td>
                <td className="text-right px-2">{fmtMoney(vgvPlanta)}</td>
              ...
              <tr className="border-b border-stone-100">
                ...
                {(["planta", "target", "liquidacao", "recompra"] as const).map((c) => (
                  <td key={c} className="text-right px-2">
              ...
              <tr className="border-b border-stone-100">
                ...
                <td className="text-right px-2">3 (fixo)</td>
                <td className="text-right px-2">7 (fixo)</td>
                <td className="text-right px-2">10 (fixo)</td>
                <td className="text-right px-2">16 (fixo)</td>
              </tr>
              <tr className="border-b border-stone-100">
                <td className="py-1.5 pr-4 text-stone-600">% Permuta</td>
                {(["permuta_planta", "permuta_target", "permuta_liquidacao", "permuta_recompra"] as const).map((k) => (
                  <td key={k} className="text-right px-2">
                    <input
                      type="number"
                      step={0.01}
                      min={0}
      ...
      {/* BLOCO 6: Funding */}
      <section className="rounded-xl border border-stone-200 bg-white p-4">
        <h3 className="text-lg font-semibold text-stone-800 mb-3">Funding</h3>
        <div className="grid gap-3 sm:grid-cols-2 md:grid-cols-4">
          <div>
            ...
            <input
              ...
              onChange={(e) => updateInput("percentual_funding", (Number(e.target.value) || 100) / 100)}
            />
          </div>
          <div>
            ...
            <input
              ...
              onChange={(e) => updateInput("cdi_an", (Number(e.target.value.replace(",", ".")) || 0) / 100)}
            />
          </div>
          <div>
            <label className="block text-sm font-medium text-stone-700">Taxa juros a.a.</label>
            <p className="mt-1 text-sm font-medium">{results ? fmtPct(results.taxa_juros_an) : "—"}</p>
          </div>
          <div>
      ...
      {results && (
        <section className="rounded-xl border border-stone-200 bg-white p-4 overflow-x-auto">
          <h3 className="text-lg font-semibold text-stone-800 mb-3">Resultados (somente leitura)</h3>
          <table className="w-full min-w-[700px] text-sm">
            <thead>
              <tr className="border-b border-stone-200">
                <th className="text-left py-2 pr-4 font-medium text-stone-700">Métrica</th>
                <th className="text-right py-2 px-2 font-medium text-stone-700">Planta</th>
                <th className="text-right py-2 px-2 font-medium text-stone-700">Target</th>
                <th className="text-right py-2 px-2 font-medium text-stone-700">Liquidação</th>
                <th className="text-right py-2 px-2 font-medium text-stone-700">Recompra</th>
              ...
              {(
                ...
                <tr key={label} className="border-b border-stone-100">
                  ...
                  {(["planta", "target", "liquidacao", "recompra"] as const).map((nome) => (
                    <td key={nome} className="text-right px-2 py-1">
                      <ValCell v={fn(results[nome])} isPct={isPct} bold={label === "Res. total Frank"} />
                    </td>
                  ))}
                </tr>
              ))}
      ...
      {temTresModelos ? (
        <>
          <section className="rounded-xl border border-stone-200 bg-white p-4">
            <h3 className="text-lg font-semibold text-stone-800 mb-3">Opções de BCA (3 modelos)</h3>
            <p className="text-sm text-stone-600 mb-4">
              Título e descrição de cada opção para envio. Salvos automaticamente.
            </p>
            <div className="grid gap-4 sm:grid-cols-1 md:grid-cols-3">
              {opcoes.map((op, index) => (
                <div key={op.catalogo_casa_id} className="rounded-lg border border-stone-200 bg-stone-50 p-4">
                  <h4 className="text-sm font-medium text-stone-700 mb-2">Opção {index + 1}</h4>
                  <input
                    type="text"
                    className="w-full rounded border border-stone-300 p-2 text-sm mb-2"
                    value={op.titulo}
        ...
        <div className="rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">
          Para preencher as 3 opções de BCA e marcar esta etapa como concluída, conclua antes a <strong>Etapa 7</strong> (Listagem, modelo e batalha) para escolher os 3 modelos do catálogo.
        </div>
      )}
    </div>
  );
