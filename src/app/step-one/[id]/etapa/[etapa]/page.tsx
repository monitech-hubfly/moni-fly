...
export default async function EtapaPage({ params }: PageProps) {
  ...
  const { data: processo } = await supabase
    ...
    .select("id, cidade, estado, ultima_validacao_casas_manuais_em, observacoes_praca, cidade_ibge_cod, anexos_etapa1, pdf_url_etapa1")
    .eq("id", id)
    .eq("user_id", user.id)
    .single();
  ...
  let etapaProgresso: { dados_json?: { narrativa?: string; analise_ibge?: unknown }; status?: string } | null = null;
  if (etapaNum === 1) {
  ...
  let condominiosEtapa2: { id: string; nome: string; qtd_casas: number | null; preco_medio: number | null; m2_medio: number | null }[] = [];
  ...
  if (etapaNum === 2) {
    ...
    const { data: ep2 } = await supabase.from("etapa_progresso").select("status").eq("processo_id", id).eq("etapa_id", 2).eq("user_id", user.id).single();
    etapa2Concluida = ep2?.status === "concluida";
  }
  let condominiosEtapa3: { id: string; nome: string; ordem: number }[] = [];
  let etapa3Resumo: Record<string, { estoque_casas?: string; ticket_lote?: string; ticket_casas?: string; ticket_casas_m2?: string; estimativa_vendidas_ano?: string }> = {};
  let etapa3Conclusao: { mais_promissores?: string; faixa_preco?: string; produto_mais_vende?: string; erros?: string; oportunidade?: string } = {};
  let etapa3Concluida = false;
  if (etapaNum === 3) {
    const { data: condList } = await supabase.from("processo_condominios").select("id, nome, ordem").eq("processo_id", id).order("ordem", { ascending: true });
    condominiosEtapa3 = (condList ?? []).map((c) => ({ id: c.id, nome: c.nome, ordem: c.ordem ?? 1 }));
    const { data: ep3 } = await supabase.from("etapa_progresso").select("dados_json, status").eq("processo_id", id).eq("etapa_id", 3).eq("user_id", user.id).single();
    const d3 = (ep3?.dados_json as { resumo_condominios?: Record<string, unknown>; conclusao?: Record<string, string> } | null) ?? {};
  ...
  let catalogoEtapa5: { id: string; nome: string | null; quartos: number | null; banheiros: number | null; vagas: number | null; preco_venda_m2: number | null }[] = [];
  let batalhasEtapa5: { casa_escolhida_id: string; listing_id: string; nota_preco: number | null; nota_produto: number | null; nota_localizacao: number | null; nota_final: number | null }[] = [];
  let lotes: { id: string; condominio: string | null; area_lote_m2: number | null; preco: number | null; preco_m2: number | null; link: string | null }[] = [];
  let catalogo: { id: string; nome: string | null; area_m2: number | null; quartos: number | null; preco_venda: number | null; preco_venda_m2: number | null }[] = [];
  let loteEscolhido: { cidade: string | null; condominio: string | null; recuos_permitidos: string | null; localizacao_condominio: string | null; area_lote_m2: number | null; topografia: string | null; frente_m: number | null; fundo_m: number | null; preco: number | null; preco_m2: number | null } | null = null;
  let batalhas: { listing_casa_id: string; catalogo_casa_id: string; nota_preco: number | null; nota_produto: number | null; nota_localizacao: number | null }[] = [];
  ...
  if (etapaNum === 4 || etapaNum === 7) {
    const { data } = await supabase.from("listings_lotes").select("id, condominio, area_lote_m2, preco, preco_m2, link, valor_condominio, iptu, caracteristicas_condominio, caracteristicas, manual").eq("processo_id", id).order("created_at", { ascending: false });
    lotes = (data ?? []) as typeof lotes;
    const { data: le } = await supabase.from("lote_escolhido").select("listing_lote_id").eq("processo_id", id).maybeSingle();
  ...
  if (etapaNum === 5 || etapaNum === 6) {
    const { data } = await supabase
      .from("listings_casas")
      .select("id, cidade, foto_url, status, condominio, localizacao_condominio, quartos, banheiros, vagas, piscina, marcenaria, preco, area_casa_m2, preco_m2, estado, compatibilidade_moni, data_publicacao, data_despublicado, link, manual")
      .eq("processo_id", id)
    ...
    const { data: batalhas5 } = await supabase
      .from("batalha_casas")
      .select("casa_escolhida_id, listing_id, nota_preco, nota_produto, nota_localizacao, nota_final, atributos_lote_json, preco_dados_json, produto_dados_json")
      .eq("processo_id", id);
  ...
  if (etapaNum === 10) {
    const [ceListRes, catListRes, ep10Res, bcaRes] = await Promise.all([
      supabase.from("catalogo_escolhidos").select("catalogo_casa_id, ordem").eq("processo_id", id).order("ordem", { ascending: true }),
      supabase.from("catalogo_casas").select("id, nome, area_m2, quartos, preco_venda").eq("ativo", true),
      supabase.from("etapa_progresso").select("dados_json").eq("processo_id", id).eq("etapa_id", 10).eq("user_id", user.id).single(),
      getBcaInputs(id),
    ]);
    catalogoEscolhidos = (ceListRes.data ?? []) as typeof catalogoEscolhidos;
    catalogo = (catListRes.data ?? []) as typeof catalogo;
    const dados10 = (ep10Res.data?.dados_json as { opcoes?: { catalogo_casa_id: string; titulo: string; descricao?: string }[] } | null) ?? {};
  ...
  if (etapaNum === 11) {
    const { data: ep1 } = await supabase.from("etapa_progresso").select("dados_json").eq("processo_id", id).eq("etapa_id", 1).eq("user_id", user.id).single();
    const narrativa11 = (ep1?.dados_json as { narrativa?: string } | null)?.narrativa ?? "";
    const { data: le11 } = await supabase.from("lote_escolhido").select("condominio, area_lote_m2, preco, preco_m2").eq("processo_id", id).single();
    const { data: batList } = await supabase.from("batalhas").select("listing_casa_id, catalogo_casa_id, nota_preco, nota_produto, nota_localizacao").eq("processo_id", id);
    const { data: ceList } = await supabase.from("catalogo_escolhidos").select("catalogo_casa_id, ordem").eq("processo_id", id).order("ordem", { ascending: true });
    const { data: catList } = await supabase.from("catalogo_casas").select("id, nome").eq("ativo", true);
    const { data: ep9 } = await supabase.from("etapa_progresso").select("dados_json").eq("processo_id", id).eq("etapa_id", 9).eq("user_id", user.id).single();
    const { data: ep10 } = await supabase.from("etapa_progresso").select("dados_json").eq("processo_id", id).eq("etapa_id", 10).eq("user_id", user.id).single();
    const justificativas11 = (ep9?.dados_json as { justificativas?: Record<string, string> } | null)?.justificativas ?? {};
    const opcoes10 = (ep10?.dados_json as { opcoes?: { titulo: string; descricao?: string }[] } | null)?.opcoes ?? [];
    ...
    resumoEtapa11 = {
      ...
      lote: le11 ? { condominio: le11.condominio, area_lote_m2: le11.area_lote_m2, preco: le11.preco, preco_m2: le11.preco_m2 } : null,
      ranking,
  ...
  return (
    <div className="min-h-screen bg-stone-50">
      <header className="border-b border-stone-200 bg-white print:hidden">
        <div className="mx-auto flex h-14 max-w-7xl items-center gap-4 px-4 flex-wrap">
      ...
      <main className="mx-auto max-w-7xl px-4 py-8">
        <div className="card">
          ...
          {isEtapa1 ? (
            ...
            <Etapa5Lotes listagemOnly processoId={id} lotes={lotes} loteEscolhidoId={null} cidadeInicial={processo?.cidade ?? ""} estadoInicial={processo?.estado ?? ""} />
          ) : etapaNum === 7 ? (
            <Etapa5Lotes processoId={id} lotes={lotes} loteEscolhidoId={loteEscolhidoIdEtapa4} cidadeInicial={processo?.cidade ?? ""} estadoInicial={processo?.estado ?? ""} />
          ) : etapaNum === 5 ? (
            <>
              <h1 className="text-xl font-bold text-moni-dark">
                Etapa {etapaDisplayNum} — {etapaInfo.nome}
              ...
              <div className="mt-6 rounded-xl border border-stone-200 bg-white shadow-sm overflow-hidden">
                <Etapa4Casas
                  ...
                  ultimaValidacaoCasasManuaisEm={processo?.ultima_validacao_casas_manuais_em ?? null}
            ...
            <>
              <div className="flex flex-wrap items-start justify-between gap-4">
                ...
                <div id="etapa6-resultado-batalha" className="w-full sm:w-auto sm:min-w-[320px] sm:max-w-[360px]" />
              </div>
