...
export default async function HomePage() {
  ...
  let processos: { id: string; cidade: string; estado: string | null; status: string; created_at: string }[] = [];
      ...
      {!user && (
        <header className="border-b border-stone-200/80 bg-white/90 backdrop-blur-sm">
          <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4">
            <Link href="/" className="text-xl font-semibold tracking-tight text-moni-primary hover:text-moni-secondary">
              Viabilidade Moní
            </Link>
        ...
        {user ? (
          <>
            ...
            <section className="mt-8">
              ...
              {processos.length === 0 ? (
                <p className="mt-4 rounded-2xl border border-stone-200/80 bg-white p-5 text-stone-600 shadow-sm">
                  Nenhum processo ainda. Use <strong>Step One em construção → Iniciar Step One</strong> no menu para começar.
                </p>
              ) : (
                    ...
                    <li key={p.id}>
                      ...
                      >
                        ...
                        <span className="text-sm text-stone-500">
                          {p.status === "em_andamento" ? "Em andamento" : p.status === "concluido" ? "Concluído" : "Rascunho"}
                        </span>
                      </Link>
                    </li>
                  ))}
            ...
            <section className="mt-8">
              ...
              {ticketsAbertos.length === 0 ? (
                <p className="mt-4 rounded-2xl border border-stone-200/80 bg-white p-5 text-stone-600 shadow-sm">
                  Nenhum ticket em aberto. Use <strong>Jurídico</strong> no menu para abrir uma nova dúvida.
                </p>
              ) : (
                <ul className="mt-4 space-y-2">
                  {ticketsAbertos.map((t) => (
                    <li key={t.id}>
                      <Link
                        ...
                        className="step-card flex items-center justify-between"
                      >
                        <span className="font-medium text-stone-900 line-clamp-1">{t.titulo}</span>
                        <span className="shrink-0 text-sm text-stone-500">{getStatusLabel(t.status)}</span>
                      </Link>
                    </li>
                  ))}
                </ul>
          ...
          <section className="text-center">
            ...
            <p className="mx-auto mt-4 max-w-2xl text-lg text-stone-600">
              Solução completa para validar sua praça e formatar a hipótese com dados — da análise ao PDF para aprovação.
            </p>
        ...
        <section className="mt-20">
          ...
          <div className="mt-8 grid gap-6 sm:grid-cols-3">
            <div className="rounded-2xl border border-stone-200/80 bg-white p-6 text-center shadow-sm">
              <span className="inline-flex h-10 w-10 items-center justify-center rounded-xl bg-moni-light text-moni-accent font-semibold">1</span>
              <h3 className="mt-3 font-semibold text-moni-dark">Análise estruturada</h3>
              <p className="mt-1.5 text-sm text-stone-600">
                Praça, condomínios &gt;5MM, checklist e tabela resumo. Dados IBGE e espaço para Atlas e Google em breve.
              </p>
            </div>
            <div className="rounded-2xl border border-stone-200/80 bg-white p-6 text-center shadow-sm">
              <span className="inline-flex h-10 w-10 items-center justify-center rounded-xl bg-moni-light text-moni-accent font-semibold">2</span>
              <h3 className="mt-3 font-semibold text-moni-dark">Batalhas e ranking</h3>
              <p className="mt-1.5 text-sm text-stone-600">
                Listagens de casas e lotes, catálogo Moní, lote escolhido. Batalhas preço/produto/localização e ranking.
              </p>
            </div>
            <div className="rounded-2xl border border-stone-200/80 bg-white p-6 text-center shadow-sm">
              <span className="inline-flex h-10 w-10 items-center justify-center rounded-xl bg-moni-light text-moni-accent font-semibold">3</span>
        ...
        <section className="mt-16">
          ...
          <div className="mt-6 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            <Link href={user ? "/step-one" : "/login"} className="step-card group block">
              <span className="text-xs font-semibold uppercase tracking-wider text-moni-accent">Etapas 1–3</span>
            ...
            <Link href={user ? "/step-one" : "/login"} className="step-card group block">
              <span className="text-xs font-semibold uppercase tracking-wider text-moni-accent">Etapas 4–7</span>
            ...
            <Link href={user ? "/step-one" : "/login"} className="step-card group block">
              <span className="text-xs font-semibold uppercase tracking-wider text-moni-accent">Etapas 8–11</span>
