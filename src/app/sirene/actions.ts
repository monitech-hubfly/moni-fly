...
export async function concluirTopico(
  ...
  // Verifica se todos os tópicos do chamado foram concluídos/aprovados
  const todosTopicos = await buscarTopicosStatusChamado(adminSync, topico.chamado_id);
  if (todosTopicosFechados(todosTopicos)) {
    // Busca o abridor do chamado para transferir o card da Pastelaria
    const { data: chamadoAbertura } = await adminSync
      .from('sirene_chamados')
      .select('aberto_por')
      .eq('id', topico.chamado_id)
      .single();

    const abertoPor = (chamadoAbertura as { aberto_por?: string } | null)?.aberto_por;

    const { data: vinculoCard } = await adminSync
      .from('sirene_pastelaria_vinculos')
      .select('pastelaria_card_id')
      .eq('sirene_chamado_id', topico.chamado_id)
      .maybeSingle();

    const cardId = (vinculoCard as { pastelaria_card_id?: string } | null)?.pastelaria_card_id;

    if (cardId) {
      let transferido = false;

      if (abertoPor) {
        // Tenta reatribuir o card ao abridor
        const { data: vinculoPessoa } = await adminSync
          .from('area_pessoas_users')
          .select('area_pessoa_id')
          .eq('user_id', abertoPor)
          .maybeSingle();

        const openerAreaPessoaId = (vinculoPessoa as { area_pessoa_id?: string } | null)?.area_pessoa_id;

        if (openerAreaPessoaId) {
          const { data: openerPessoa } = await adminSync
            .from('area_pessoas')
            .select('nome')
            .eq('id', openerAreaPessoaId)
            .maybeSingle();

          await adminSync
            .from('pastelaria_cards')
            .update({
              responsavel_id: openerAreaPessoaId,
              responsavel_nome: (openerPessoa as { nome?: string } | null)?.nome ?? null,
              coluna: 'inbox',
              updated_at: new Date().toISOString(),
            })
            .eq('id', cardId);
      ...
      if (!transferido) {
        // Abridor não tem area_pessoa — move para done para sair do backlog do responsável atual
        await adminSync
          .from('pastelaria_cards')
          .update({ coluna: 'done', updated_at: new Date().toISOString() })
          .eq('id', cardId);
      }

      revalidatePath('/carometro/pastelaria');
      revalidatePath('/carometro/todo-planning');
    }
  }
