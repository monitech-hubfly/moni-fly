    ...
    <div className="min-h-screen bg-gradient-to-b from-stone-50 via-white to-stone-50">
      <header className="border-b border-stone-200/80 bg-white/90 backdrop-blur-sm">
        <div className="mx-auto flex h-14 max-w-7xl items-center gap-4 px-4">
          <Link href={isMoni ? "/juridico/kanban" : "/juridico"} className="text-moni-primary hover:underline">
            ← {isMoni ? "Kanban Jurídico" : "Jurídico"}
          </Link>
          <span className="text-stone-400">/</span>
          <span className="font-medium text-stone-700 truncate">{ticket.titulo}</span>
        </div>
      </header>
      ...
      <main className="mx-auto max-w-7xl px-4 py-8">
          ...
          <div className="border-b border-stone-200 bg-stone-50/80 px-5 py-4">
            ...
            {(ticket.nome_frank || ticket.nome_condominio || ticket.lote) && (
              <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-sm text-stone-600">
                {ticket.nome_frank && <span><strong>Nome:</strong> {ticket.nome_frank}</span>}
                {ticket.nome_condominio && <span><strong>Condomínio:</strong> {ticket.nome_condominio}</span>}
                {ticket.lote && <span><strong>Lote:</strong> {ticket.lote}</span>}
              </div>
            )}
          ...
          <div className="p-5 space-y-5">
            ...
            {ticket.resposta_publica && (
              <div className="rounded-xl border border-emerald-200 bg-emerald-50/50 p-4">
                ...
                <p className="mt-1 whitespace-pre-wrap text-emerald-900">{ticket.resposta_publica}</p>
            ...
            <div>
              <h2 className="text-sm font-medium text-stone-600 mb-2">Anexos</h2>
              {anexos.length === 0 ? (
        ...
        <JuridicoTicketDetailClient ticketId={id} isMoni={isMoni} status={ticket.status} comentarios={comentarios} />
      </main>
      <MoniFooter />
    </div>
  );
