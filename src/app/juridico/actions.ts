...
> {
  ...
  const { data: tickets, error } = await supabase
    .from('juridico_tickets')
    .select(
      'id, titulo, descricao, status, created_at, resposta_publica, nome_frank, nome_condominio, lote',
    )
...
> {
  ...
  const { data: ticket, error } = await supabase
    .from('juridico_tickets')
    .select(
      'id, user_id, titulo, descricao, status, resposta_publica, resposta_publica_em, created_at, updated_at, nome_frank, nome_condominio, lote',
    )
    .eq('id', ticketId)
    .single();
...
export async function setJuridicoRespostaPublica(
  ...
  const mensagem =
    'Sua dúvida jurídica foi respondida. Acesse o canal de dúvidas (Jurídico) para ver a resposta e anexos.';
  await supabase.from('alertas').insert({
    user_id: ticket.user_id,
    tipo: 'Dúvida jurídica',
    mensagem,
...
export async function listJuridicoComentariosInternos(
  ticketId: string,
): Promise<
...
export async function registerJuridicoAnexo(
  ...
  if (!user) return { ok: false, error: 'Faça login.' };
  const role = await getMyRole(supabase);
  if (lado === 'moni' && role !== 'consultor' && role !== 'admin')
...
export async function getJuridicoAnexoDownloadUrl(
  anexoId: string,
  ...
  const path = anexo.file_url.startsWith('http')
    ? anexo.file_url.replace(/^.*\/juridico-anexos\//, '')
    : anexo.file_url;
