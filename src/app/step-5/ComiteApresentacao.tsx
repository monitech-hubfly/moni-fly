...
export function ComiteApresentacao({ data }: { data: ComiteData }) {
  ...
  return (
    <div className="space-y-8">
      ...
      <section className="overflow-hidden rounded-xl border border-stone-200 bg-white print:break-inside-avoid">
        ...
        <div className="min-h-[400px]">
          {data.pdfUrlProspeccao ? (
            <iframe
              ...
              className="h-[600px] w-full print:hidden"
            />
          ) : (
      ...
      <section className="overflow-hidden rounded-xl border border-stone-200 bg-white print:break-inside-avoid">
        ...
        <div className="p-6">
          {data.resumo ? (
            <>
              ...
              {data.resumo.lote && (
                <div className="mb-4">
                  ...
                  <p className="text-sm text-stone-600">
                    Condomínio: {data.resumo.lote.condominio ?? '—'} | Área:{' '}
                    {data.resumo.lote.area_lote_m2 ?? '—'} m² | Preço:{' '}
                    {data.resumo.lote.preco != null
                      ? data.resumo.lote.preco.toLocaleString('pt-BR')
                      : '—'}{' '}
                    | R$/m²:{' '}
                    {data.resumo.lote.preco_m2 != null
                      ? data.resumo.lote.preco_m2.toLocaleString('pt-BR')
                      : '—'}
                  </p>
                </div>
              )}
            ...
            <p className="text-sm text-stone-500">Dados do resumo ainda não disponíveis.</p>
          )}
        </div>
      </section>
    </div>
