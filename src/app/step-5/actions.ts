 ...
 */
export async function getComiteData(processoId: string, ownerUserId?: string): Promise<ComiteData | null> {
  ...
  const ids = [...ce11].sort((a, b) => (a.ordem ?? 0) - (b.ordem ?? 0)).map((ce) => ce.catalogo_casa_id);
  ...
  const resumo: ResumoComite = {
    ...
    narrativa,
    lote: le ? { condominio: le.condominio, area_lote_m2: le.area_lote_m2, preco: le.preco, preco_m2: le.preco_m2 } : null,
    ranking,
