...
type Props = {
  templateHref: string;
  importar: (csvText: string) => Promise<Resultado>;
  atualizar: (csvText: string) => Promise<Resultado>;
  tituloImportar?: string;
  tituloAtualizar?: string;
};

export function ImportarEntidadeCSVButton({
  templateHref,
  importar,
  atualizar,
  tituloImportar = 'Adiciona linhas novas',
  tituloAtualizar = 'Atualiza linhas existentes; células vazias não apagam dados',
}: Props) {
  ...
  const importFile = async (file: File) => {
    ...
    try {
      ...
      const result = modo === 'atualizar' ? await atualizar(text) : await importar(text);
      if (result.ok) {
  ...
  return (
    <>
      <div className="flex shrink-0 items-center gap-2">
        <a href={templateHref} download className={redeBtnGhost}>
