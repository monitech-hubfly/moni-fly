...
export async function GET(
  ...
  if (instError || !instance) return NextResponse.json({ error: "Documento não encontrado" }, { status: 404 });
  ...
  if (role === "consultor" && processo?.user_id) {
    const { data: frank } = await supabase.from("profiles").select("id").eq("id", processo.user_id).eq("consultor_id", user.id).single();
    isConsultor = !!frank;
  }
