...
export async function POST(req: Request) {
  ...
  if (typeof processoId !== 'string' || !file || !(file instanceof File)) {
    return NextResponse.redirect(new URL('/step-7', req.url));
  }
  ...
  try {
    ...
    const templatePath = tpl?.arquivo_path
      ? tpl.arquivo_path.includes('/')
        ? tpl.arquivo_path
        : `step7/${tpl.arquivo_path}`
      : null;
  ...
  if (uploadError) {
    return NextResponse.redirect(new URL('/step-7?erroUpload=1', req.url));
  }
