...
function toUF(estado: string | null | undefined): string | null {
  ...
  const mapa: Record<string, string> = {
    ...
    RONDONIA: "RO", RORAMA: "RR", "SANTA CATARINA": "SC", "SAO PAULO": "SP",
...
async function buscarCodigoMunicipio(
  ...
  if (!municipio) {
    return { ok: false, error: `Município "${cidade}" não encontrado na UF ${uf}. Verifique o nome ou a UF.` };
  }
...
function extrairValorAgregadoV3(json: unknown, periodo: string = "2021"): number | null {
  try {
    const obj = json as { resultados?: Array<{ series?: Record<string, Array<{ serie?: Record<string, unknown> }>> }> };
  ...
  try {
      ...
      try {
        ...
        if (sidraRes.ok) {
          ...
          if (Array.isArray(arr) && arr.length > 0) {
            const v = (arr[0] as Record<string, unknown>).V ?? (arr[0] as Record<string, unknown>).valor;
            if (v != null) popVal = typeof v === "number" ? v : parseFloat(String(v).replace(/\D/g, "").replace(",", "."));
          }
        }
      } catch (e2) {
    ...
    try {
      ...
      if (Array.isArray(rendaArr) && rendaArr.length > 0) {
        const v = (rendaArr[0] as Record<string, unknown>).V ?? (rendaArr[0] as Record<string, unknown>).valor;
        if (v != null) rendaVal = typeof v === "number" ? v : parseFloat(String(v).replace(/\D/g, "").replace(",", "."));
      }
    } catch (e) {
