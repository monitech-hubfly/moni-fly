...
const ALL_KANBAN_EXPORT_FIELDS: KanbanExportFieldDef[] = [
  ...
  field('franqueado.regional', 'Regional', 'franqueado', 7, (ctx) => displayOrDash(ctx.detalhes.rede?.regional), (v) => !v.isLoteador && !v.isFunding),
  field('franqueado.area_atuacao', 'Área de atuação', 'franqueado', 8, (ctx) => displayOrDash(ctx.detalhes.rede?.area_atuacao), (v) => !v.isLoteador && !v.isFunding),
  field('franqueado.email', 'E-mail', 'franqueado', 9, (ctx) => displayOrDash(ctx.detalhes.rede?.email_frank), (v) => !v.isLoteador && !v.isFunding),
  field('franqueado.telefone', 'Telefone', 'franqueado', 10, (ctx) => displayOrDash(ctx.detalhes.rede?.telefone_frank), (v) => !v.isLoteador && !v.isFunding),
  field('franqueado.cpf', 'CPF', 'franqueado', 11, (ctx) => displayOrDash(ctx.detalhes.rede?.cpf_frank), (v) => !v.isLoteador && !v.isFunding),
  field('franqueado.nascimento', 'Nascimento', 'franqueado', 12, (ctx) => fmtDataExport(ctx.detalhes.rede?.data_nasc_frank), (v) => !v.isLoteador && !v.isFunding),
  field('franqueado.responsavel_comercial', 'Responsável comercial', 'franqueado', 13, (ctx) => displayOrDash(ctx.detalhes.rede?.responsavel_comercial), (v) => !v.isLoteador && !v.isFunding),
  field('franqueado.camiseta', 'Camiseta', 'franqueado', 14, (ctx) => displayOrDash(ctx.detalhes.rede?.tamanho_camisa_frank), (v) => !v.isLoteador && !v.isFunding),
  field('franqueado.ass_cof', 'Ass. COF', 'franqueado', 15, (ctx) => fmtDataExport(ctx.detalhes.rede?.data_ass_cof), (v) => !v.isLoteador && !v.isFunding),
  field('franqueado.ass_contrato', 'Ass. Contrato', 'franqueado', 16, (ctx) => fmtDataExport(ctx.detalhes.rede?.data_ass_contrato), (v) => !v.isLoteador && !v.isFunding),
  field('franqueado.expiracao', 'Expiração', 'franqueado', 17, (ctx) => fmtDataExport(ctx.detalhes.rede?.data_expiracao_franquia), (v) => !v.isLoteador && !v.isFunding),
  field('franqueado.endereco', 'Endereço (casa)', 'franqueado', 18, (ctx) => enderecoCasaRede(ctx.detalhes.rede), (v) => !v.isLoteador && !v.isFunding),
  field('franqueado.socios', 'Sócios', 'franqueado', 19, (ctx) => displayOrDash(ctx.detalhes.rede?.socios), (v) => !v.isLoteador && !v.isFunding),
  ...
  field('kanban.responsavel_fase', 'Responsável do card', 'kanban', 5, (ctx) =>
