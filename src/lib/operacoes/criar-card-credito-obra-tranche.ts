...
export async function criarCardCreditoObraTranche(
  ...
  try {
    ...
    try {
      ...
      if (errAtiv) {
        console.error('[criarCardCreditoObraTranche] atividade:', errAtiv.message);
      }
    } catch (e) {
      console.error('[criarCardCreditoObraTranche] atividade throw:', e);
    }
    ...
    const msg = e instanceof Error ? e.message : String(e);
    console.error('[criarCardCreditoObraTranche] unexpected:', msg);
    return { ok: false, error: msg || 'Erro inesperado ao criar card Crédito Obra.' };
  }
}
