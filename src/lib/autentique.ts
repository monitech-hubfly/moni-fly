...
export async function createDocument(
  ...
  const blob = new Blob([fileBuffer], {
    type: fileName.toLowerCase().endsWith(".pdf") ? "application/pdf" : "application/vnd.openxmlformats-officedocument.wordprocessingml.document",
  });
  ...
  const json = (await response.json()) as { data?: { createDocument?: CreateDocumentResult }; errors?: Array<{ message?: string }> };
  if (json.errors?.length) {
    return { ok: false, error: json.errors.map((e) => e.message).join(" ") || "Erro na API Autentique." };
  }
