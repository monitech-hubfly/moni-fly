...
function mergeSetAreaUrl(setSearchParams, nextAreaId) {
  ...
  useEffect(() => {
    ...
    listarAreas(supabase, 'id, nome').then(({ data }) => {
      ...
      if (next && next !== areaFromUrl) {
        mergeSetAreaUrl(setSearchParams, next)
      }
    })
    return () => { cancel = true }
...
ALTER TABLE indicadores
  ...
  useEffect(() => {
    ...
    abrirMetaModal()
    mergeSetAreaUrl(setSearchParams, areaId)
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  }, [editAcaoFromUrl, areaId, tarefas, areas, setSearchParams])
            ...
            <div className="workload-area-field">
              ...
              <select
                ...
                onChange={e => {
                  const v = e.target.value
                  setAreaId(v)
                  if (v) mergeSetAreaUrl(setSearchParams, v)
                }}
                aria-label="Área"
              >
