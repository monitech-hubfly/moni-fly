...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!resDev.ok) {
      ...
      return new Response(
        JSON.stringify({ error: `Glue-API DEVELOPMENT ${resDev.status}`, detail: text.slice(0, 300) }),
        { status: resDev.status, headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
    }

    const dataUsed = (await resUsed.json()) as { search?: { totalCount?: number; result?: { listings?: unknown[] } }; page?: unknown };
    const dataDev = (await resDev.json()) as { search?: { totalCount?: number; result?: { listings?: unknown[] } }; page?: unknown };

    const usedListings = dataUsed.search?.result?.listings ?? [];
    const devListings = dataDev.search?.result?.listings ?? [];
    const combinedListings = [...(Array.isArray(usedListings) ? usedListings : []), ...(Array.isArray(devListings) ? devListings : [])];
