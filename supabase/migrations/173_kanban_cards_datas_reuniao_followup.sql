-- Datas de reunião e follow-up em cards nativos e processos (legado via view).

ALTER TABLE public.kanban_cards
  ADD COLUMN IF NOT EXISTS data_reuniao date,
  ADD COLUMN IF NOT EXISTS data_followup date;

COMMENT ON COLUMN public.kanban_cards.data_reuniao IS 'Data planejada de reunião (card nativo).';
