-- 360: Funil Loteadores — Execução do Material: renomear link apresentação e ocultar legados.
...
BEGIN
  ...
  IF v_kanban_id IS NULL THEN
    RAISE NOTICE '360: kanban Funil Loteadores não encontrado; pulando.';
    RETURN;
  END IF;
  ...
  IF v_fase_id IS NULL THEN
    RAISE NOTICE '360: fase execucao_material_moni_inc não encontrada; pulando.';
    RETURN;
  END IF;

  UPDATE public.kanban_fase_checklist_itens
  SET label = 'Link da Apresentação',
      config_json = COALESCE(config_json, '{}'::jsonb) - 'oculto_ui',
      obrigatorio = false
  WHERE fase_id = v_fase_id
    AND campo_slug = 'materiais_visuais_apresentacao';
