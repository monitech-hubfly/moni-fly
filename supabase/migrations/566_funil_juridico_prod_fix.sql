-- 566: Funil Jurídico — PROD fix + SLA atualizado.
-- Contexto: migration 565 usava UUID placeholder (35fb5c8d-0000-0000-0000-000000000001).
--   Em DEV o kanban já existia com UUID real (35fb5c8d-50c0-4999-bc16-89d53c2e758f).
--   Em PROD nem o kanban nem a 565 foram aplicados.
-- Esta migration:
--   (a) Marca a 565 como aplicada (evita re-run com UUID errado).
--   (b) Cria/atualiza o kanban com o UUID canônico correto.
--   (c) Cria/atualiza as 8 fases com os SLAs definidos pelo time Jurídico.
-- Idempotente. Segura para rodar em DEV e PROD.

-- ─── Marca 565 como aplicada (no-op se já existe) ────────────────────────────
    ...
    ALTER TABLE public.kanban_cards
      ...
      CHECK (
        ...
        OR juridico_tipo_contrato IN (
          'opcao', 'cto_com_permuta', 'cto_sem_permuta',
          'showroom', 'parceria', 'franquia', 'cof'
        )
      );
  END IF;
END $$;
...
-- ─── Kanban Funil Jurídico (UUID canônico) ───────────────────────────────────
-- UUID correto: 35fb5c8d-50c0-4999-bc16-89d53c2e758f
-- Cria somente se não existe nenhum kanban com este UUID OU com o nome.
INSERT INTO public.kanbans (id, nome, descricao, ativo, cor_hex)
SELECT
  '35fb5c8d-50c0-4999-bc16-89d53c2e758f'::uuid,
  'Funil Jurídico',
...
-- Garante que está ativo e com dados corretos caso já existisse.
...
-- Remove o kanban com UUID placeholder se existir (limpeza DEV).
DELETE FROM public.kanbans
WHERE id = '35fb5c8d-0000-0000-0000-000000000001'::uuid;

-- ─── Fases (8 etapas, sla_tipo = uteis) ──────────────────────────────────────
-- SLAs validados pelo time Jurídico:
--   Recebimento         → NULL (entrada, sem prazo)
--   Análise Inicial     → 2 d.u.
--   Em alts e respostas → 2 d.u.
--   Enviado ao Parceiro → 3 d.u.
--   Subir p/ Assinatura → 2 d.u.
--   Aguardando Assinat. → 3 d.u.
--   Pós-Assinatura      → 5 d.u.
--   Atendimentos Conc.  → NULL (terminal)
...
-- Atualiza fases existentes (inclui correção de SLA em DEV).
...
FROM public.kanbans k,
  (VALUES
    ('juridico_recebimento',            'Recebimento',                       1, NULL::integer, false, 'Card recebido via bastão (Portfólio/Loteadores) ou manualmente (Comercial). Confirmar tipo de contrato e origem antes de avançar.'),
    ('juridico_analise_inicial',        'Análise Inicial',                   2, 2,             false, 'Analisar viabilidade jurídica. Confirmar documentos necessários. Verificar pendências no checklist do card pai.'),
    ('juridico_alteracoes_respostas',   'Em alterações e respostas',         3, 2,             false, 'Conduzir tratativas. Documentar rodadas. Registrar versões nos comentários. Se o parceiro retornar com alterações após o envio, retroceder para cá — contador de revisões será incrementado.'),
    ('juridico_enviado_parceiro',       'Enviado ao Parceiro',               4, 3,             false, 'Enviar minuta ao parceiro. Registrar data e confirmação de recebimento. Se houver novas alterações, retroceder para Fase 3.'),
    ('juridico_subir_assinatura',       'Subir p/ Assinatura',               5, 2,             false, 'Gerar versão final no Autentique. Colar link do envelope no card. Confirmar que todas as partes receberam o convite.'),
    ('juridico_aguardando_assinaturas', 'Aguardando Assinaturas',            6, 3,             false, 'Aguardar assinaturas. Monitorar no Autentique. Cobrar partes pendentes. Avançar somente quando 100% assinado.'),
    ('juridico_pos_assinatura',         'Pós-Assinatura / Retroalimentação', 7, 5,             false, 'Baixar contrato assinado e anexar ao card. Avaliar aprendizados. Marcar decisão sobre retroalimentação (obrigatória antes de avançar — gate ativo). Se sim: atualizar template e FAQ.'),
    ('juridico_atendimentos_concluidos','Atendimentos Concluídos',           8, NULL::integer, true,  'Terminal. Nenhuma ação adicional. Card é registro histórico.')
...
-- ─── Registro ────────────────────────────────────────────────────────────────
INSERT INTO supabase_migrations.schema_migrations (version, name)
VALUES ('566', 'funil_juridico_prod_fix')
ON CONFLICT (version) DO NOTHING;
