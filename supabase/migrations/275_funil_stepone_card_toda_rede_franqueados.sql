-- 275: Funil Step One — card para cada linha de rede_franqueados (backfill idempotente).
...
BEGIN
  ...
  IF v_kanban_id IS NULL THEN
    RAISE NOTICE '275: kanban Funil Step One não encontrado; pulando.';
    RETURN;
  END IF;
  ...
  WHERE kanban_id = v_kanban_id
    AND slug IN ('dados_candidato', 'stepone_dados_candidato')
    AND COALESCE(ativo, true) = true
  ORDER BY ordem ASC
  LIMIT 1;

  IF v_fase_id IS NULL THEN
    RAISE NOTICE '275: fase Dados do Candidato não encontrada; pulando.';
    RETURN;
  END IF;
  ...
  LOOP
    ...
    -- Repara card órfão (sem rede_franqueado_id) antes de inserir duplicata.
    ...
    WHERE kc.id = (
      ...
      WHERE k2.kanban_id = v_kanban_id
        AND k2.franqueado_id = v_user_id
  ...
  RAISE NOTICE '275: cards criados/reparados no Funil Step One: %', v_criados;
END;
$$;
