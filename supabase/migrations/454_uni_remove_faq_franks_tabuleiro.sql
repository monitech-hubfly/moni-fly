-- 454_uni_remove_faq_franks_tabuleiro.sql
-- Remove a FAQ da estrutura acadêmica do Tabuleiro (casa "faq-franks" + seus módulos).
-- O conteúdo já foi migrado para faq_categories/faq_articles em 453_faq_seed_conteudo.sql.
...
-- Idempotente. Reversível: re-executar 453 recria o conteúdo na FAQ; para restaurar a casa,
...
begin
  ...
  if v_faq_count = 0 then
    raise exception 'Abortado: não há artigos em faq_articles. Rode 453_faq_seed_conteudo.sql antes de remover a casa.';
  end if;
