-- 532: Funil Loteadores — primeira fase passa a se chamar Entrar em contato.
...
BEGIN
  ...
  IF v_kanban_id IS NULL THEN
    RAISE NOTICE '532: Funil Loteadores não encontrado — pulando.';
    RETURN;
  END IF;

  UPDATE public.kanban_fases
  SET nome = 'Entrar em contato'
  WHERE kanban_id = v_kanban_id
    AND slug IN ('primeiro_contato_moni_inc', 'loteador_cadastro')
    AND nome IS DISTINCT FROM 'Entrar em contato';
END $$;
