-- 285: Funil Step One — reestrutura checklist Dados dos Condomínios (global vs faixas).
...
BEGIN
  ...
  IF v_fase_id IS NULL THEN
    RAISE NOTICE '285: fase dados_condominios não encontrada; pulando.';
    RETURN;
  END IF;
  ...
  SET
    ordem = 1,
    placeholder = 'Use as abas por condomínio: caracterização e lotes (globais) e, em cada faixa (Premium, Intermediária, Entrada), Liquidez e Valorização Exponencial.'
...
3. Faixas de produto — preencha as três abas:
   ...
   Em cada faixa: Liquidez e Valorização Exponencial (construções, casas prontas, preços, tempo de venda, vendas nos últimos 12 meses, remanescentes e locação).
